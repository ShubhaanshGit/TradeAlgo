...
def algo_trade_3(amount, user_input):
    ...
    asset_n = user_asset_input # Initialise the asset_n variable
    count = 0
...
st.title('CRYPTO MANIA')
st.subheader("Who said crypto doesn't make sense!")
...
def beautifydf(df):
    ...
    df = df.style.format("{:.10}")


# In[14]:


user_asset_input = st.selectbox(
'Choose a base cryptocurrency ticker of your choice - ', l1)

st.write('Your selected option - ', user_asset_input)
    
button = st.button('Get profitable flips')

base_asset_amount = st.slider('Choose base asset amount',
                        min_value = 0.00000000, max_value = 100000000.00, step = 0.00000001)
st.write('Chose amount of base asset = ', base_asset_amount)

if button:
    prof_df = algo_trade_3(base_asset_amount, user_asset_input)
    beautifydf(prof_df)
